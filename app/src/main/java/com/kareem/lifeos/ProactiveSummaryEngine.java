...
final class ProactiveSummaryEngine {
    ...
    static Result summarizeConversation(String label,List<LifeDb.Event> events){
        ...
        for(LifeDb.Event e:events)if(EventSemantics.isPersonConversation(e)&&!"Visible conversation".equals(e.title)){granular=true;break;}
        for(LifeDb.Event e:events){
            if(!EventSemantics.isPersonConversation(e))continue;
            String raw=clean(e.body);if(raw.isEmpty())continue;
            for(OpenLoopExtractor.Candidate candidate:OpenLoopExtractor.extract(e.title,raw,e.at))if(EventSemantics.supportsLoop(e,candidate.kind)){signalSet.add(candidate.kind);priority=Math.max(priority,Math.max(1,candidate.priority/20));}
            if(raw.contains("?")&&raw.length()>=8){signalSet.add("question");priority=Math.max(priority,2);}if(URL.matcher(raw).find())signalSet.add("link");
            if(granular&&"Visible conversation".equals(e.title))continue;
            for(String s:split(raw)){String x=normalize(stripSender(s,label));if(noise(x)||x.length()<5||x.length()>220)continue;sentences.add(x);break;}
            if(!sentences.isEmpty())break;
        }
        String summary=join(sentences,1);if(summary.isEmpty())summary="No substantive recent message could be summarized.";else summary="Latest · "+summary;
        String why="";if(signalSet.contains("deadline"))why="A dated obligation may need action.";else if(signalSet.contains("request"))why="A direct request may need a response.";else if(signalSet.contains("commitment"))why="A commitment may still need follow-up.";else if(signalSet.contains("appointment"))why="A scheduled plan was mentioned.";else if(signalSet.contains("question"))why="A question appears in the conversation.";
