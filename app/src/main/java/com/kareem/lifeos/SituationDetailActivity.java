...
public final class SituationDetailActivity extends Activity {
    ...
    @Override public void onCreate(Bundle s){super.onCreate(s);db=new LifeDb(this);situationId=getIntent().getStringExtra("situation_id");}
    @Override protected void onResume(){super.onResume();load();render();}
    ...
    private void render(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setBackgroundColor(BG);LinearLayout top=new LinearLayout(this);top.setGravity(Gravity.CENTER_VERTICAL);top.setPadding(dp(12),dp(10),dp(16),dp(8));top.setBackgroundColor(SURFACE);Button back=plain("‹");back.setContentDescription("Back");back.setOnClickListener(v->finish());top.addView(back,new LinearLayout.LayoutParams(dp(48),dp(48)));TextView title=text("Situation",22,TEXT);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);top.addView(title);root.addView(top);root.addView(divider());ScrollView sc=new ScrollView(this);LinearLayout body=new LinearLayout(this);body.setOrientation(LinearLayout.VERTICAL);body.setPadding(dp(16),dp(18),dp(16),dp(28));sc.addView(body);root.addView(sc,new LinearLayout.LayoutParams(-1,0,1));if(situation==null){body.addView(text("This situation is no longer active.",14,MUTED));setContentView(root);return;}
        body.addView(badge(situation.status));TextView h=text(situation.title,22,TEXT);h.setTypeface(Typeface.DEFAULT,Typeface.BOLD);h.setPadding(0,dp(10),0,0);body.addView(h);body.addView(meta("Updated "+time(situation.latestAt)));
        ...
        body.addView(section("Recent evidence"));List<LifeDb.Event> es=new ArrayList<>(SituationEngine.evidence(db,situation,40));int shown=Math.min(12,es.size());for(int i=0;i<shown;i++){LifeDb.Event e=es.get(i);LinearLayout c=row();TextView t=text((e.title==null||e.title.trim().isEmpty()||"Visible conversation".equals(e.title)?LifeDb.personLabel(e):e.title)+"  ›",14,TEXT);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);c.addView(t);c.addView(text(clip(e.body,220),13,TEXT));c.addView(meta(time(e.at)));c.setOnClickListener(v->startActivity(new Intent(this,EvidenceDetailActivity.class).putExtra("event_id",e.id).putExtra("mode","situation")));body.addView(c);}if(es.size()>shown)body.addView(meta((es.size()-shown)+" older evidence items are kept in the conversation history."));setContentView(root);}
