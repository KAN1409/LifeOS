...
final class PresentationSemantics {
    ...
    static String title(Context c,LifeDb.Event e){
        ...
        if(carBooking(all,m)){String subject=m==null?"":subjectFromSummary(m.summary);if(usefulSubject(subject))return UserFacingText.humanize(subject);String source=cleanLabel(e.title);if(usefulSubject(source)&&isMostlyLatin(source))return UserFacingText.humanize(source);return "Car care booking";}
    ...
    static String summary(Context c,LifeDb.Event e){
        ...
        if(educationPayment(all,m)){
            ...
            return "First installment still needs payment.";
        }
        if(carBooking(all,m)&&m==null)return "Car details are needed to confirm the booking.";
    ...
    static boolean meaningfulTimeline(Context c,LifeDb.Event e){
        if(e==null)return false;String x=norm(e.title+" "+e.body);if(platformActivity(x)||looksPromotional(x))return false;NotificationMeaning m=meaning(c,e);
        if(m!=null){
            ...
            if("PERSON_CONVERSATION".equals(m.type)&&"INFORMATIONAL".equals(m.state)&&"INFORMATION".equals(m.intent))return meaningfulMessageText(e.body);
        }
        return meaningfulMessageText(e.body)&&!clean(summary(c,e)).isEmpty();
    }

    static String timelineFingerprint(Context c,LifeDb.Event e){return norm(e.app+"|"+title(c,e)+"|"+summary(c,e));}
    ...
    private static boolean platformActivity(String value){String x=norm(value);return x.contains("is typing")||x.contains("typing")||x.contains("replied to your chat")||x.contains("reacted to")||x.contains("liked your message")||x.contains("sent you a snap")||x.contains("added to their story")||x.contains("following")||x.contains("voice note from snapchat")||x.contains("received a voice note")||x.contains("new snap from");}
    ...
    private static boolean usefulSubject(String s){String x=norm(s);return !x.isEmpty()&&!x.equals("you")&&!x.equals("user")&&!x.equals("info")&&!x.equals("information")&&!x.equals("notification")&&!x.equals("message")&&!x.equals("gmail")&&!x.equals("whatsapp");}
