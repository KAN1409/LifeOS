...
final class PresentationSemantics {
    ...
    static String attentionFingerprint(Context c,LifeDb.Event e){if(e==null)return "";NotificationMeaning m=meaning(c,e);String all=clean(e.title)+" "+clean(e.body);if(educationPayment(all,m))return "school-fee-payment";if(carBooking(all,m))return "car-booking|"+norm(e.threadKey+"|"+LifeDb.personLabel(e));return norm(title(c,e)+"|"+summary(c,e));}
