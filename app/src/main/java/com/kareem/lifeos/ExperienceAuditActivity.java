...
public final class ExperienceAuditActivity extends Activity {
    ...
    private void begin(){List<ExperienceAudit.Target> xs=new ArrayList<>();
        ...
        try{LifeDb db=new LifeDb(this);LifeDb.Event seed=conversationSeed(db);if(seed!=null){xs.add(new ExperienceAudit.Target("12 Conversation Detail",new Intent(this,ConversationDetailActivity.class).putExtra("event_id",seed.id)));xs.add(new ExperienceAudit.Target("13 Evidence Detail",new Intent(this,EvidenceDetailActivity.class).putExtra("event_id",seed.id).putExtra("mode","activity")));}List<SituationEngine.Situation> s=SituationEngine.build(db,db.openLoops(50),new com.kareem.lifeos.actions.PersistentActionQueue(this).pending(),System.currentTimeMillis());if(!s.isEmpty())xs.add(new ExperienceAudit.Target("14 Situation Detail",new Intent(this,SituationDetailActivity.class).putExtra("situation_id",s.get(0).id)));db.close();}catch(Throwable ignored){}
