...
public final class ExperienceAuditActivity extends Activity {
    ...
    private void begin(){List<ExperienceAudit.Target> xs=new ArrayList<>();
        ...
        xs.add(target("10 Life Intelligence Timeline",SecondBrainActivity.class,null,null));
        xs.add(target("11 Life Intelligence People",SecondBrainActivity.class,null,null));
        xs.add(target("12 Life Intelligence Attention",SecondBrainActivity.class,null,null));
        xs.add(target("13 Life Intelligence Briefing",SecondBrainActivity.class,null,null));
        xs.add(target("14 Action Center Pending",ActionCenterActivity.class,null,null));
        xs.add(target("15 Action Center History",ActionCenterActivity.class,null,null));
        try{LifeDb db=new LifeDb(this);LifeDb.Event seed=conversationSeed(db);if(seed!=null){xs.add(new ExperienceAudit.Target("16 Conversation Detail",new Intent(this,ConversationDetailActivity.class).putExtra("event_id",seed.id)));xs.add(new ExperienceAudit.Target("17 Evidence Detail",new Intent(this,EvidenceDetailActivity.class).putExtra("event_id",seed.id).putExtra("mode","activity")));xs.add(new ExperienceAudit.Target("18 Person Detail",new Intent(this,PersonDetailActivity.class).putExtra("event_id",seed.id)));}List<SituationEngine.Situation> s=SituationEngine.build(db,db.openLoops(50),new com.kareem.lifeos.actions.PersistentActionQueue(this).pending(),System.currentTimeMillis());if(!s.isEmpty())xs.add(new ExperienceAudit.Target("19 Situation Detail",new Intent(this,SituationDetailActivity.class).putExtra("situation_id",s.get(0).id)));db.close();}catch(Throwable ignored){}
        ExperienceAudit.start(this,xs);continueAudit();
    }
    private LifeDb.Event conversationSeed(LifeDb db){List<LifeDb.Event> es=db.recentEvents(120);for(LifeDb.Event e:es)if(LifeDb.isConversationLike(e)&&e.app!=null&&e.app.contains("whatsapp"))return e;for(LifeDb.Event e:es)if(LifeDb.isConversationLike(e))return e;return es.isEmpty()?null:es.get(0);}
