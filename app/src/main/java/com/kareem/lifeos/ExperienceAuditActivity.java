...
public final class ExperienceAuditActivity extends Activity {
    ...
    private void render(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(20),dp(24),dp(20),dp(24));root.setBackgroundColor(BG);TextView h=t("Full LifeOS QA",25,TEXT);h.setTypeface(Typeface.DEFAULT,Typeface.BOLD);root.addView(h);TextView sub=t("Runs non-destructive runtime/provider checks, then automatically visits product and diagnostic screens, captures pixels + hierarchy + runtime state, and exports one QA ZIP.",14,MUTED);sub.setPadding(0,dp(8),0,dp(18));root.addView(sub);state=t("Ready",13,GREEN);root.addView(state);Button run=button("Run full LifeOS test",true);run.setOnClickListener(v->begin());LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,dp(50));p.setMargins(0,dp(20),0,0);root.addView(run,p);Button back=button("Back",false);back.setOnClickListener(v->finish());LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(-1,dp(48));bp.setMargins(0,dp(10),0,0);root.addView(back,bp);TextView info=t("Output: qa_report.json + screenshots (.png) + screen structure (.json) + runtime/data snapshot (.json), saved as one ZIP in Downloads/LifeOS. The test does not mark commitments handled or create/delete user data.",12,MUTED);info.setPadding(0,dp(20),0,0);root.addView(info);setContentView(root);}

    private void begin(){
        FullQaHarness.Report pre=FullQaHarness.run(this);state.setText("Runtime checks · "+pre.pass+" pass · "+pre.fail+" fail · "+pre.warn+" warn");
