...
final class LifeOsCapabilityRegistry {
    static final class Capability {final String id,label,description,icon,query;final int color,count;final String countLabel,secondary;Capability(String id,String label,String description,String icon,int color,String query,int count,String countLabel,String secondary){this.id=id;this.label=label;this.description=description;this.icon=icon;this.color=color;this.query=query;this.count=Math.max(0,count);this.countLabel=countLabel==null?"":countLabel;this.secondary=secondary==null?"":secondary;}String primaryLine(){return count+" "+countLabel;}}
    ...
    private static String kind(String id){if("people".equals(id))return "People";if("conversations".equals(id))return "Conversation";if("decisions".equals(id))return "Decision";if("voice".equals(id))return "Voice Memory";if("files".equals(id))return "File";if("places".equals(id))return "Place";if("projects".equals(id))return "Project";if("events".equals(id))return "Event";if("commitments".equals(id))return "Commitment";return "Object";}
    private static String countLabel(String id,int count){if("people".equals(id))return count==1?"person":"people";if("conversations".equals(id))return count==1?"conversation":"conversations";if("decisions".equals(id))return count==1?"decision":"decisions";if("voice".equals(id))return count==1?"recording":"recordings";if("files".equals(id))return count==1?"file":"files";if("places".equals(id))return count==1?"place":"places";if("projects".equals(id))return count==1?"project":"projects";if("events".equals(id))return count==1?"event":"events";if("commitments".equals(id))return count==1?"commitment":"commitments";return count==1?"item":"items";}
}
