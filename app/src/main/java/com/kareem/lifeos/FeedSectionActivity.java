...
public final class FeedSectionActivity extends Activity {
    ...
    private void load(){content.removeAllViews();if("attention".equals(mode)){List<LifeDb.Loop> xs=db.openLoops(100);for(LifeDb.Loop x:xs){LinearLayout c=row();c.addView(badge(x.kind.toUpperCase()));TextView t=text(x.title,15,TEXT);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);t.setPadding(0,dp(7),0,0);c.addView(t);LifeDb.Event e=db.eventById(x.evidenceId);if(e!=null)c.addView(meta(app(e.app)+" · "+time(e.at)));c.setOnClickListener(v->startActivity(new Intent(this,EvidenceDetailActivity.class).putExtra("event_id",x.evidenceId).putExtra("loop_id",x.id).putExtra("mode","attention")));content.addView(c);}if(xs.isEmpty())empty();return;}if("actions".equals(mode)){List<PersistentActionQueue.Item> xs=new PersistentActionQueue(this).pending();for(PersistentActionQueue.Item x:xs){LinearLayout c=row();c.addView(badge(x.proposal.actionType.toUpperCase()));TextView t=text(x.proposal.payloadSummary,15,TEXT);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);t.setPadding(0,dp(7),0,0);c.addView(t);c.addView(meta(x.proposal.target));c.setOnClickListener(v->startActivity(new Intent(this,ActionCenterActivity.class)));content.addView(c);}if(xs.isEmpty())empty();return;}List<LifeDb.Event> es=db.recentEvents(100);for(LifeDb.Event e:es){LinearLayout c=row();TextView t=text(e.title==null||e.title.isEmpty()?app(e.app):e.title,15,TEXT);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);c.addView(t);TextView b=text(e.body==null?"":e.body,13,TEXT);b.setPadding(0,dp(6),0,0);c.addView(b);c.addView(meta(app(e.app)+" · "+time(e.at)));c.setOnClickListener(v->startActivity(new Intent(this,EvidenceDetailActivity.class).putExtra("event_id",e.id).putExtra("mode","activity")));content.addView(c);}if(es.isEmpty())empty();}
    private void empty(){TextView v=text("Nothing here right now.",14,MUTED);v.setPadding(dp(4),dp(8),0,0);content.addView(v);}private LinearLayout row(){LinearLayout c=new LinearLayout(this);c.setOrientation(LinearLayout.VERTICAL);c.setPadding(dp(14),dp(13),dp(14),dp(13));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,-2);p.setMargins(0,0,0,dp(8));c.setLayoutParams(p);c.setBackground(round(SURFACE,BORDER,9));return c;}private TextView badge(String s){TextView v=text(s,11,GREEN);v.setTypeface(Typeface.DEFAULT,Typeface.BOLD);v.setPadding(dp(8),dp(4),dp(8),dp(4));v.setBackground(round(Color.TRANSPARENT,GREEN,20));return v;}private TextView meta(String s){TextView v=text(s,11,MUTED);v.setPadding(0,dp(7),0,0);return v;}private Button button(String s){Button b=new Button(this);b.setText(s);b.setTextColor(TEXT);b.setTextSize(18);b.setAllCaps(false);b.setBackgroundColor(Color.TRANSPARENT);return b;}private View divider(){View v=new View(this);v.setBackgroundColor(BORDER);v.setLayoutParams(new LinearLayout.LayoutParams(-1,dp(1)));return v;}private GradientDrawable round(int fill,int stroke,int radius){GradientDrawable g=new GradientDrawable();g.setColor(fill);g.setCornerRadius(dp(radius));g.setStroke(dp(1),stroke);return g;}private TextView text(String s,int size,int color){TextView v=new TextView(this);v.setText(s);v.setTextSize(size);v.setTextColor(color);v.setLineSpacing(0,1.08f);return v;}private int dp(int n){return Math.round(n*getResources().getDisplayMetrics().density);}private static String app(String p){if(p==null)return "Unknown";int i=p.lastIndexOf('.');String x=i>=0?p.substring(i+1):p;return x.isEmpty()?p:Character.toUpperCase(x.charAt(0))+x.substring(1);}private static String time(long at){return DateFormat.getDateTimeInstance(DateFormat.SHORT,DateFormat.SHORT).format(new Date(at));}
}
