...
final class LifeOsUi {
    ...
    static View orb(Context c){return new View(c){final Paint ring=new Paint(Paint.ANTI_ALIAS_FLAG);final Paint glow=new Paint(Paint.ANTI_ALIAS_FLAG);{setLayerType(View.LAYER_TYPE_SOFTWARE,null);} @Override protected void onDraw(Canvas canvas){super.onDraw(canvas);float cx=getWidth()/2f,cy=getHeight()/2f;float r=Math.min(getWidth(),getHeight())*.32f;int[] colors={BLUE,BLUE,Color.rgb(88,118,255),PURPLE,BLUE};float[] pos={0f,.45f,.70f,.86f,1f};Shader shader=new SweepGradient(cx,cy,colors,pos);glow.setStyle(Paint.Style.STROKE);glow.setStrokeWidth(dp(getContext(),5));glow.setShader(shader);glow.setMaskFilter(new BlurMaskFilter(dp(getContext(),16),BlurMaskFilter.Blur.NORMAL));glow.setAlpha(145);canvas.drawCircle(cx,cy,r,glow);glow.setMaskFilter(null);ring.setStyle(Paint.Style.STROKE);ring.setStrokeCap(Paint.Cap.ROUND);ring.setStrokeWidth(dp(getContext(),2));ring.setShader(shader);ring.setAlpha(255);canvas.drawCircle(cx,cy,r,ring);}};}
