...
final class LifeOsUi {
    ...
    static TextView text(Context c,String s,float size,int color){TextView v=new TextView(c);v.setText(s==null?"":s);v.setTextSize(size);v.setTextColor(color);v.setTypeface(Typeface.create("sans-serif",Typeface.NORMAL));v.setLineSpacing(0,1.04f);v.setIncludeFontPadding(false);return v;}
    ...
    static LinearLayout card(Context c){LinearLayout r=new LinearLayout(c);r.setOrientation(LinearLayout.VERTICAL);r.setPadding(dp(c,13),dp(c,12),dp(c,13),dp(c,12));r.setBackground(round(c,SURFACE,BORDER,15));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,-2);p.setMargins(0,0,0,dp(c,8));r.setLayoutParams(p);return r;}
    static TextView badge(Context c,String label,int color){TextView v=text(c,label==null?"":label.toUpperCase(),9,color);weight(v,700);v.setPadding(dp(c,7),dp(c,3),dp(c,7),dp(c,3));v.setBackground(round(c,Color.TRANSPARENT,color,18));v.setLayoutParams(new LinearLayout.LayoutParams(-2,-2));return v;}
    static Button button(Context c,String label){Button b=new Button(c);b.setText(label);b.setAllCaps(false);b.setTextSize(12);b.setTextColor(TEXT);b.setTypeface(Typeface.create("sans-serif",Typeface.NORMAL));b.setGravity(Gravity.CENTER);b.setMinHeight(0);b.setMinimumHeight(0);b.setMinWidth(0);b.setMinimumWidth(0);b.setPadding(dp(c,12),0,dp(c,12),0);b.setBackground(round(c,SURFACE_2,BORDER,14));b.setStateListAnimator(null);return b;}
    static Button primary(Context c,String label){Button b=button(c,label);b.setBackground(round(c,BLUE,BLUE,14));b.setTextColor(Color.WHITE);weight(b,600);return b;}
    static Button chip(Context c,String label,boolean active){Button b=button(c,label);b.setTextSize(10.5f);b.setPadding(dp(c,13),0,dp(c,13),0);b.setBackground(round(c,active?BLUE:SURFACE_2,active?BLUE:BORDER,20));b.setTextColor(active?Color.WHITE:TEXT);b.setTypeface(Typeface.create(active?"sans-serif-medium":"sans-serif",Typeface.NORMAL));return b;}
    static TextView section(Context c,String label){TextView t=text(c,label,16.5f,TEXT);weight(t,700);t.setPadding(dp(c,1),dp(c,16),0,dp(c,8));return t;}
    ...
    static LinearLayout topBar(Activity a,String title,boolean settings){LinearLayout h=new LinearLayout(a);h.setGravity(Gravity.CENTER_VERTICAL);h.setPadding(dp(a,16),dp(a,10),dp(a,10),dp(a,8));h.setBackgroundColor(BG);TextView t=text(a,title,23,TEXT);weight(t,700);h.addView(t,new LinearLayout.LayoutParams(0,-2,1));if(settings){View gear=iconTouch(a,LifeOsIconView.MORE,TEXT,22,42);gear.setContentDescription("Settings and diagnostics");gear.setOnClickListener(v->a.startActivity(new Intent(a,MainActivity.class)));h.addView(gear,new LinearLayout.LayoutParams(dp(a,42),dp(a,42)));}return h;}
    ...
    static TextView avatar(Context c,String initial){TextView v=text(c,initial,13.5f,TEXT);weight(v,500);v.setGravity(Gravity.CENTER);v.setBackground(round(c,SURFACE_2,BORDER,100));return v;}
    ...
    static View orb(Context c){return new View(c){final Paint ring=new Paint(Paint.ANTI_ALIAS_FLAG);final Paint glow=new Paint(Paint.ANTI_ALIAS_FLAG);{setLayerType(View.LAYER_TYPE_SOFTWARE,null);} @Override protected void onDraw(Canvas canvas){super.onDraw(canvas);float cx=getWidth()/2f,cy=getHeight()/2f;float r=Math.min(getWidth(),getHeight())*.36f;int[] colors={BLUE,BLUE,Color.rgb(88,118,255),PURPLE,BLUE};float[] pos={0f,.45f,.70f,.86f,1f};Shader shader=new SweepGradient(cx,cy,colors,pos);glow.setStyle(Paint.Style.STROKE);glow.setStrokeWidth(dp(getContext(),5));glow.setShader(shader);glow.setMaskFilter(new BlurMaskFilter(dp(getContext(),18),BlurMaskFilter.Blur.NORMAL));glow.setAlpha(155);canvas.drawCircle(cx,cy,r,glow);glow.setMaskFilter(null);ring.setStyle(Paint.Style.STROKE);ring.setStrokeCap(Paint.Cap.ROUND);ring.setStrokeWidth(dp(getContext(),3));ring.setShader(shader);ring.setAlpha(255);canvas.drawCircle(cx,cy,r,ring);}};}

    static LinearLayout bottomNav(Activity a,Class<?> active){LinearLayout wrap=new LinearLayout(a);wrap.setOrientation(LinearLayout.VERTICAL);wrap.setBackgroundColor(SURFACE);wrap.addView(divider(a));LinearLayout row=new LinearLayout(a);row.setGravity(Gravity.CENTER);row.setPadding(dp(a,2),dp(a,3),dp(a,2),dp(a,4));row.addView(nav(a,LifeOsIconView.HOME,"Now",active==FeedActivity.class,FeedActivity.class),new LinearLayout.LayoutParams(0,dp(a,60),1));row.addView(nav(a,LifeOsIconView.TIMELINE,"Timeline",active==TimelineActivity.class,TimelineActivity.class),new LinearLayout.LayoutParams(0,dp(a,60),1));row.addView(nav(a,LifeOsIconView.SEARCH,"Search",active==SearchActivity.class,SearchActivity.class),new LinearLayout.LayoutParams(0,dp(a,60),1));row.addView(nav(a,LifeOsIconView.ASK,"Ask",active==AskLifeOsActivity.class,AskLifeOsActivity.class),new LinearLayout.LayoutParams(0,dp(a,60),1));wrap.addView(row);return wrap;}
    private static LinearLayout nav(Activity a,String icon,String label,boolean active,Class<?> target){LinearLayout v=new LinearLayout(a);v.setOrientation(LinearLayout.VERTICAL);v.setGravity(Gravity.CENTER);int color=active?BLUE:TERTIARY;LifeOsIconView iv=new LifeOsIconView(a,icon,color);v.addView(iv,new LinearLayout.LayoutParams(dp(a,22),dp(a,22)));TextView t=text(a,label,9.5f,color);weight(t,active?600:400);t.setGravity(Gravity.CENTER);LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-2,-2);lp.topMargin=dp(a,3);v.addView(t,lp);v.setOnClickListener(x->go(a,target));return v;}
