...
final class LifeOsUi {
    ...
    static final int TEXT=Color.rgb(242,245,249),MUTED=Color.rgb(160,169,180),TERTIARY=Color.rgb(120,132,146);
    ...
    static TextView text(Context c,String s,float size,int color){TextView v=new TextView(c);v.setText(s==null?"":s);v.setTextSize(refSp(size));v.setTextColor(color);v.setTypeface(Typeface.create("sans-serif",Typeface.NORMAL));v.setLineSpacing(0,1.03f);v.setIncludeFontPadding(false);return v;}
    ...
    static Button button(Context c,String label){Button b=new Button(c);b.setText(label);b.setAllCaps(false);b.setTextSize(refSp(11.5f));b.setTextColor(TEXT);b.setTypeface(Typeface.create("sans-serif-medium",Typeface.NORMAL));b.setGravity(Gravity.CENTER);b.setMinHeight(0);b.setMinimumHeight(0);b.setMinWidth(0);b.setMinimumWidth(0);b.setPadding(dp(c,11),0,dp(c,11),0);b.setBackground(round(c,SURFACE_2,BORDER,13));b.setStateListAnimator(null);return b;}
    ...
    static Button chip(Context c,String label,boolean active){Button b=button(c,label);b.setTextSize(refSp(10.2f));b.setPadding(dp(c,12),0,dp(c,12),0);b.setBackground(round(c,active?BLUE:SURFACE_2,active?BLUE:BORDER,18));b.setTextColor(active?Color.WHITE:TEXT);b.setTypeface(Typeface.create("sans-serif-medium",Typeface.NORMAL));return b;}
    ...
    private static LinearLayout nav(Activity a,String icon,String label,boolean active,Class<?> target){LinearLayout v=new LinearLayout(a);v.setOrientation(LinearLayout.VERTICAL);v.setGravity(Gravity.CENTER);int color=active?BLUE:TERTIARY;LifeOsIconView iv=new LifeOsIconView(a,icon,color);v.addView(iv,new LinearLayout.LayoutParams(dp(a,22),dp(a,22)));TextView t=text(a,label,9.1f,color);weight(t,active?600:500);t.setGravity(Gravity.CENTER);LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-2,-2);lp.topMargin=dp(a,3);v.addView(t,lp);v.setOnClickListener(x->go(a,target));return v;}
