...
final class LifeOsUi {
    static final int BG=Color.rgb(7,11,16),SURFACE=Color.rgb(17,24,32),SURFACE_2=Color.rgb(27,36,47),BORDER=Color.rgb(37,47,58);
    static final int TEXT=Color.rgb(238,242,246),MUTED=Color.rgb(145,154,166),TERTIARY=Color.rgb(103,114,128);
    ...
    static TextView text(Context c,String s,float size,int color){TextView v=new TextView(c);v.setText(s==null?"":s);v.setTextSize(size);v.setTextColor(color);v.setTypeface(Typeface.create("sans-serif",Typeface.NORMAL));v.setLineSpacing(0,1.025f);v.setIncludeFontPadding(false);return v;}
    ...
    static Button chip(Context c,String label,boolean active){Button b=button(c,label);b.setTextSize(10.2f);b.setPadding(dp(c,12),0,dp(c,12),0);b.setBackground(round(c,active?BLUE:SURFACE_2,active?BLUE:BORDER,18));b.setTextColor(active?Color.WHITE:TEXT);b.setTypeface(Typeface.create(active?"sans-serif-medium":"sans-serif",Typeface.NORMAL));return b;}
    ...
    static LinearLayout topBar(Activity a,String title,boolean settings){LinearLayout h=new LinearLayout(a);h.setGravity(Gravity.CENTER_VERTICAL);h.setPadding(dp(a,16),dp(a,8),dp(a,10),dp(a,7));h.setBackgroundColor(BG);TextView t=text(a,title,20.5f,TEXT);weight(t,700);h.addView(t,new LinearLayout.LayoutParams(0,-2,1));if(settings){View gear=iconTouch(a,LifeOsIconView.MORE,TEXT,20,38);gear.setContentDescription("Settings and diagnostics");gear.setOnClickListener(v->a.startActivity(new Intent(a,MainActivity.class)));h.addView(gear,new LinearLayout.LayoutParams(dp(a,38),dp(a,38)));}return h;}
    static Button iconButton(Context c,String glyph){Button b=button(c,glyph);b.setTextSize(17);b.setPadding(0,0,0,0);b.setBackgroundColor(Color.TRANSPARENT);return b;}
    ...
    static View orb(Context c){return new View(c){final Paint ring=new Paint(Paint.ANTI_ALIAS_FLAG);final Paint glow=new Paint(Paint.ANTI_ALIAS_FLAG);{setLayerType(View.LAYER_TYPE_SOFTWARE,null);} @Override protected void onDraw(Canvas canvas){super.onDraw(canvas);float cx=getWidth()/2f,cy=getHeight()/2f;float r=Math.min(getWidth(),getHeight())*.32f;int[] colors={BLUE,BLUE,Color.rgb(88,118,255),PURPLE,BLUE};float[] pos={0f,.45f,.70f,.86f,1f};SweepGradient shader=new SweepGradient(cx,cy,colors,pos);Matrix matrix=new Matrix();matrix.setRotate(88f,cx,cy);shader.setLocalMatrix(matrix);glow.setStyle(Paint.Style.STROKE);glow.setStrokeWidth(dp(getContext(),5));glow.setShader(shader);glow.setMaskFilter(new BlurMaskFilter(dp(getContext(),16),BlurMaskFilter.Blur.NORMAL));glow.setAlpha(145);canvas.drawCircle(cx,cy,r,glow);glow.setMaskFilter(null);ring.setStyle(Paint.Style.STROKE);ring.setStrokeCap(Paint.Cap.ROUND);ring.setStrokeWidth(dp(getContext(),2));ring.setShader(shader);ring.setAlpha(255);canvas.drawCircle(cx,cy,r,ring);}};}

    static LinearLayout bottomNav(Activity a,Class<?> active){LinearLayout wrap=new LinearLayout(a);wrap.setOrientation(LinearLayout.VERTICAL);wrap.setBackgroundColor(SURFACE);wrap.addView(divider(a));LinearLayout row=new LinearLayout(a);row.setGravity(Gravity.CENTER);row.setPadding(dp(a,2),dp(a,2),dp(a,2),dp(a,3));row.addView(nav(a,LifeOsIconView.HOME,"Now",active==FeedActivity.class,FeedActivity.class),new LinearLayout.LayoutParams(0,dp(a,55),1));row.addView(nav(a,LifeOsIconView.TIMELINE,"Timeline",active==TimelineActivity.class,TimelineActivity.class),new LinearLayout.LayoutParams(0,dp(a,55),1));row.addView(nav(a,LifeOsIconView.SEARCH,"Search",active==SearchActivity.class,SearchActivity.class),new LinearLayout.LayoutParams(0,dp(a,55),1));row.addView(nav(a,LifeOsIconView.ASK,"Ask",active==AskLifeOsActivity.class,AskLifeOsActivity.class),new LinearLayout.LayoutParams(0,dp(a,55),1));wrap.addView(row);return wrap;}
    private static LinearLayout nav(Activity a,String icon,String label,boolean active,Class<?> target){LinearLayout v=new LinearLayout(a);v.setOrientation(LinearLayout.VERTICAL);v.setGravity(Gravity.CENTER);int color=active?BLUE:TERTIARY;LifeOsIconView iv=new LifeOsIconView(a,icon,color);v.addView(iv,new LinearLayout.LayoutParams(dp(a,20),dp(a,20)));TextView t=text(a,label,9f,color);weight(t,active?600:400);t.setGravity(Gravity.CENTER);LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-2,-2);lp.topMargin=dp(a,2);v.addView(t,lp);v.setOnClickListener(x->go(a,target));return v;}
