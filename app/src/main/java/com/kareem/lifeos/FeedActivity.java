...
/** Proactive home: ranked conclusions first, plumbing hidden, everything drillable. */
public final class FeedActivity extends Activity {
    ...
    private void refresh(){
        ...
        int shown=0;
        for(ProactiveFeedEngine.Suggestion s:suggestions){if(shown++>=3)break;suggestionCard(s);}
        for(PersistentActionQueue.Item x:agentActions){if(shown++>=3)break;actionCard(x);}
        if(suggestionCount==0)empty("No useful action is being suggested right now.");

        sectionHeader("Today",ranked.size(),"activity");
        if(ranked.isEmpty()){List<LifeDb.Event> events=db.recentEvents(30);if(events.isEmpty())empty("No meaningful activity captured yet.");else for(int i=0;i<Math.min(5,events.size());i++)eventCard(events.get(i));}
        else for(int i=0;i<Math.min(5,ranked.size());i++)conversationCard(ranked.get(i));

        navCard("People & Social Radar","Relationship history, conversations and changes",LifeSignalsActivity.class);
    ...
    private void openSection(String m){startActivity(new Intent(this,FeedSectionActivity.class).putExtra("mode",m));}
    private void openEvidence(long id,String m,long loop){startActivity(new Intent(this,EvidenceDetailActivity.class).putExtra("event_id",id).putExtra("mode",m).putExtra("loop_id",loop));}
    ...
    private TextView badge(String s){TextView v=text(s,11,GREEN);v.setTypeface(Typeface.DEFAULT,Typeface.BOLD);v.setPadding(dp(8),dp(4),dp(8),dp(4));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-2,-2);p.setMargins(0,dp(7),0,0);v.setLayoutParams(p);v.setBackground(round(Color.TRANSPARENT,GREEN,20));return v;}
