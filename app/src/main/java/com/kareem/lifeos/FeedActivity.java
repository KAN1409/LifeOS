...
public final class FeedActivity extends Activity {
    ...
    private void refresh(){
        ...
        List<AttentionStore.Item> durableAttention=AttentionStore.get(this).openItems(30);
        List<LifeDb.Loop> fallbackAttention=legacyNotCovered(rawLoops,durableAttention);
        ...
        int totalAttention=durableAttention.size()+fallbackAttention.size();
        ...
        if(totalAttention==0)empty("Nothing currently needs attention.");else{
            int shown=0;
            for(AttentionStore.Item item:durableAttention){if(shown++>=3)break;attentionItemCard(item);}
            for(LifeDb.Loop x:fallbackAttention){if(shown++>=3)break;attentionCard(x);}
        }

        List<AttentionStore.Item> confirmed=confirmed(durableAttention);
        int suggestionCount=confirmed.size()+fallbackSuggestions.size()+agentActions.size();
        sectionHeader("Suggested actions",Math.min(3,suggestionCount),"actions");
        int shown=0;
        for(AttentionStore.Item item:confirmed){if(shown++>=3)break;attentionSuggestionCard(item);}
    ...
    private List<AttentionStore.Item> confirmed(List<AttentionStore.Item> items){ArrayList<AttentionStore.Item> out=new ArrayList<>();for(AttentionStore.Item x:items)if(!x.provisional&&AttentionStore.OPEN.equals(x.status))out.add(x);return out;}
    private List<LifeDb.Loop> legacyNotCovered(List<LifeDb.Loop> loops,List<AttentionStore.Item> items){Set<Long> covered=new HashSet<>();for(AttentionStore.Item item:items)covered.add(item.eventId);ArrayList<LifeDb.Loop> out=new ArrayList<>();for(LifeDb.Loop x:loops)if(!covered.contains(x.evidenceId))out.add(x);return out;}
    ...
    private void attentionItemCard(AttentionStore.Item item){LifeDb.Event e=db.eventById(item.eventId);LinearLayout c=row();String signal=item.provisional?"ANALYZING":(!"NONE".equals(item.intent)?item.intent:item.type);c.addView(badge(signal));String who=e==null?"":(LifeDb.isConversationLike(e)?LifeDb.personLabel(e):LifeDb.friendlyApp(e.app));TextView t=limited((who.isEmpty()?"":who+" · ")+item.summary+"  ›",15,TEXT,2);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);t.setPadding(0,dp(6),0,0);c.addView(t);if(!item.reason.isEmpty()){TextView why=limited(item.reason,12,GREEN,2);why.setPadding(0,dp(5),0,0);c.addView(why);}c.addView(meta(item.provisional?"Reserved instantly · deep analysis pending":actionLabel(item.action)+" · "+item.urgency.toLowerCase()+" urgency"));c.setOnClickListener(v->openEvidence(item.eventId,"attention",0));content.addView(c);}
    private void attentionSuggestionCard(AttentionStore.Item item){LinearLayout c=row();c.addView(badge(actionLabel(item.action).toUpperCase()));TextView t=limited(actionLabel(item.action)+": "+item.summary+"  ›",15,TEXT,2);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);t.setPadding(0,dp(6),0,0);c.addView(t);if(!item.reason.isEmpty())c.addView(meta(item.reason));c.setOnClickListener(v->openEvidence(item.eventId,"action",0));content.addView(c);}
