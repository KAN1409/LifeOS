...
public final class FeedActivity extends Activity {
    ...
    private void refresh(){
        ...
        if(attention.isEmpty())empty("Nothing currently needs attention.");else for(int i=0;i<Math.min(2,attention.size());i++)attentionCard(attention.get(i));

        int suggestionCount=suggestions.size()+agentActions.size();
        sectionHeader("Suggested actions",suggestionCount,"actions");
        int shown=0;for(ProactiveFeedEngine.Suggestion s:suggestions){if(shown++>=2)break;suggestionCard(s);}for(PersistentActionQueue.Item x:agentActions){if(shown++>=2)break;actionCard(x);}if(suggestionCount==0)empty("No grounded action is being suggested right now.");

        sectionHeader("Today",ranked.size(),"activity");
        if(ranked.isEmpty()){List<LifeDb.Event> events=db.recentEvents(30);if(events.isEmpty())empty("No meaningful activity captured yet.");else for(int i=0;i<Math.min(3,events.size());i++)eventCard(events.get(i));}else for(int i=0;i<Math.min(3,ranked.size());i++)conversationCard(ranked.get(i));
    ...
    private void situationHeader(int count){LinearLayout r=new LinearLayout(this);r.setGravity(Gravity.CENTER_VERTICAL);r.setPadding(dp(2),dp(14),dp(2),dp(7));TextView t=text("Situations",17,TEXT);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);r.addView(t,new LinearLayout.LayoutParams(0,-2,1));r.addView(text(count+" open",12,MUTED));content.addView(r);}
    private void situationCard(SituationEngine.Situation s){LinearLayout c=row();c.addView(badge(s.status));TextView t=limited(s.title+"  ›",16,TEXT,1);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);t.setPadding(0,dp(6),0,0);c.addView(t);TextView sm=limited(s.summary,13,TEXT,2);sm.setPadding(0,dp(5),0,0);c.addView(sm);if(s.why!=null&&!s.why.isEmpty()){TextView why=limited(s.why,12,GREEN,1);why.setPadding(0,dp(5),0,0);c.addView(why);}c.addView(meta(s.eventCount+" evidence · "+s.attentionCount+" open · "+time(s.latestAt)));c.setOnClickListener(v->startActivity(new Intent(this,SituationDetailActivity.class).putExtra("situation_id",s.id)));content.addView(c);}
