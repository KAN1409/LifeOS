...
public final class MainActivity extends Activity {
    ...
    private void render(){
        ...
        TextView subtitle=text("V2 Agent Runtime · Teya transplant + LifeOS context foundation",12,MUTED);subtitle.setPadding(0,dp(2),0,0);top.addView(subtitle);
    ...
    private void showLoops(){select(loops);content.removeAllViews();List<LifeDb.Loop> xs=db.openLoops(100);section("Attention","Things that may need action");if(xs.isEmpty()){empty("Nothing needs attention yet. The next integration batch connects LifeOS situations and SecondBrain retrieval here.");return;}for(LifeDb.Loop x:xs){LinearLayout row=row();row.addView(badge(x.kind.toUpperCase()));TextView title=text(x.title,15,TEXT);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);title.setPadding(0,dp(7),0,dp(9));row.addView(title);Button done=secondary("Mark done");done.setOnClickListener(v->{db.closeLoop(x.id);showLoops();});row.addView(done,new LinearLayout.LayoutParams(-1,dp(40)));content.addView(row);}}
