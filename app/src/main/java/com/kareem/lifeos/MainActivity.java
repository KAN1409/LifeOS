...
public final class MainActivity extends Activity {
    ...
    private void refresh(){
        ...
        int raw=UniversalObservationStore.get(this).count(),memories=PersistentLifeMemoryStore.get(this).searchable().size();boolean brain=new ConfigManager(this).isConfigured();AttentionStore a=AttentionStore.get(this);
        status.setText((enabled?"●":"○")+" notifications   "+(screen?"●":"○")+" screen context   "+(brain?"●":"○")+" agent brain\n"+raw+" raw observations  ·  "+memories+" grounded memories  ·  "+openAttentionCount()+" open attention items\n"+a.pendingCount()+" semantic items pending  ·  "+a.confirmedOpenCount()+" confirmed durable attention\nengine "+version+"  ·  Teya Harness imported  ·  SecondBrain + Graphiti + approval gate ready");
        status.setTextColor(enabled||screen||brain?GREEN:MUTED);showRecent();
    }

    private int openAttentionCount(){List<AttentionStore.Item> durable=AttentionStore.get(this).openItems(1000);return durable.size()+fallbackLoops(durable).size();}
    private List<LifeDb.Loop> fallbackLoops(List<AttentionStore.Item> durable){Set<Long> covered=new HashSet<>();for(AttentionStore.Item item:durable)covered.add(item.eventId);ArrayList<LifeDb.Loop> out=new ArrayList<>();for(LifeDb.Loop loop:db.openLoops(1000))if(!covered.contains(loop.evidenceId))out.add(loop);return out;}
    ...
    private void showLoops(){select(loops);content.removeAllViews();List<AttentionStore.Item> durable=AttentionStore.get(this).openItems(100);List<LifeDb.Loop> fallback=fallbackLoops(durable);section("Attention","Durable unresolved items · reading the source does not close them");if(durable.isEmpty()&&fallback.isEmpty()){empty("No unresolved item needs attention right now.");return;}for(AttentionStore.Item x:durable){LinearLayout row=row();row.addView(badge(x.provisional?"ANALYZING":(!"NONE".equals(x.intent)?x.intent:x.type)));LifeDb.Event e=db.eventById(x.eventId);String who=e==null?"":(LifeDb.isConversationLike(e)?LifeDb.personLabel(e):LifeDb.friendlyApp(e.app));TextView title=text((who.isEmpty()?"":who+" · ")+x.summary,15,TEXT);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);title.setPadding(0,dp(7),0,dp(9));row.addView(title);row.addView(meta("priority "+x.priority+" · confidence "+Math.round(x.confidence*100)+"%"));Button done=secondary("Mark handled");done.setOnClickListener(v->{AttentionStore.get(this).markHandled(x.eventId);db.getWritableDatabase().execSQL("UPDATE open_loops SET status='done' WHERE evidence_id=? AND status='open'",new Object[]{x.eventId});showLoops();});row.addView(done,new LinearLayout.LayoutParams(-1,dp(48)));content.addView(row);}for(LifeDb.Loop x:fallback){LinearLayout row=row();row.addView(badge(x.kind.replace('_',' ').toUpperCase()));LifeDb.Event e=db.eventById(x.evidenceId);String who=e==null?"":(LifeDb.isConversationLike(e)?LifeDb.personLabel(e):LifeDb.friendlyApp(e.app));TextView title=text((who.isEmpty()?"":who+" · ")+x.title,15,TEXT);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);title.setPadding(0,dp(7),0,dp(9));row.addView(title);row.addView(meta("priority "+x.priority+" · confidence "+Math.round(x.confidence*100)+"%"));Button done=secondary("Mark handled");done.setOnClickListener(v->{db.closeLoop(x.id);showLoops();});row.addView(done,new LinearLayout.LayoutParams(-1,dp(48)));content.addView(row);}}

    private void confirmErase(){new AlertDialog.Builder(this).setTitle("Erase preview data?").setMessage("This deletes legacy and V2 data stored by this install.").setNegativeButton("Cancel",null).setPositiveButton("Erase",(d,w)->{db.eraseAll();AttentionStore.get(this).eraseAll();NotificationMeaningStore.get(this).eraseAll();PersistentUnderstandingStore.get(this).eraseAllUnderstanding();UniversalObservationStore.get(this).eraseAll();PersistentLifeMemoryStore.get(this).eraseAll();refresh();}).show();}
