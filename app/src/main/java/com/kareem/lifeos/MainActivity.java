...
public final class MainActivity extends Activity {
    private static final int BG=Color.rgb(13,17,23),SURFACE=Color.rgb(22,27,34),BORDER=Color.rgb(48,54,61),TEXT=Color.rgb(230,237,243),MUTED=Color.rgb(139,148,158),GREEN=Color.rgb(63,185,80),RED=Color.rgb(248,81,73);
    ...
    private void render(){
        ...
        TextView subtitle=text("V2 context preview · open-source transplant checkpoint",12,MUTED);subtitle.setPadding(0,dp(2),0,0);top.addView(subtitle);
    ...
    private void refresh(){
        ...
        int raw=UniversalObservationStore.get(this).count();
        int memories=PersistentLifeMemoryStore.get(this).searchable().size();
        status.setText((enabled?"●":"○")+" notifications   "+(screen?"●":"○")+" screen context\n"+
                raw+" raw V2 observations  ·  "+memories+" grounded memories\n"+
                db.count("events")+" legacy events  ·  "+db.count("open_loops")+" attention items  ·  engine "+version+"\n"+
                "Teya retention  ·  hybrid RRF recall  ·  dedup active");
        status.setTextColor(enabled||screen?GREEN:MUTED);showRecent();
    }

    private void showRecent(){select(recent);content.removeAllViews();section("Activity","Recent captured evidence");List<LifeDb.Event> xs=db.recentEvents(100);if(xs.isEmpty()){empty("No captured activity yet. Turn on notification or screen context access to exercise the V2 observation pipeline.");return;}for(LifeDb.Event x:xs){LinearLayout row=row();TextView t=text(blank(x.title)?friendlyApp(x.app):x.title,15,TEXT);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);row.addView(t);TextView body=text(x.body,13,TEXT);body.setPadding(0,dp(5),0,0);row.addView(body);row.addView(meta(friendlyApp(x.app)+" · "+formatTime(x.at)));content.addView(row);}}

    private void showCanonical(){select(canonical);content.removeAllViews();PersistentUnderstandingStore u=PersistentUnderstandingStore.get(this);List<CanonicalEventRecord> xs=u.recentCanonical(100);long rebuilt=u.canonicalRebuiltAt();section("Understanding","Canonical shadow interpretation · raw evidence remains replayable");if(rebuilt>0)content.addView(meta("Rebuilt "+formatTime(rebuilt)+" · provenance preserved"));if(xs.isEmpty()){empty("No canonical events yet. V2 keeps interpretation separate from raw evidence and now has grounded durable-memory plumbing underneath it.");return;}for(CanonicalEventRecord x:xs){LinearLayout row=row();row.addView(badge(x.direction.name()+" · "+x.type));TextView body=text(x.text,14,TEXT);body.setPadding(0,dp(7),0,0);row.addView(body);row.addView(meta(x.sources+" · confidence "+Math.round(x.confidence*100)+"% · "+formatTime(x.observedAt)));content.addView(row);}}

    private void showLoops(){select(loops);content.removeAllViews();List<LifeDb.Loop> xs=db.openLoops(100);section("Attention","Things that may need action");if(xs.isEmpty()){empty("Nothing needs attention yet. Grounded situations and Deep Brain priorities will surface here as the V2 semantic transport is connected.");return;}for(LifeDb.Loop x:xs){LinearLayout row=row();row.addView(badge(x.kind.toUpperCase()));TextView title=text(x.title,15,TEXT);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);title.setPadding(0,dp(7),0,dp(9));row.addView(title);Button done=secondary("Mark done");done.setOnClickListener(v->{db.closeLoop(x.id);showLoops();});row.addView(done,new LinearLayout.LayoutParams(-1,dp(40)));content.addView(row);}}

    private void confirmErase(){new AlertDialog.Builder(this).setTitle("Erase preview data?").setMessage("This deletes legacy and V2 data stored by this preview install only.").setNegativeButton("Cancel",null).setPositiveButton("Erase",(d,w)->{db.eraseAll();PersistentUnderstandingStore.get(this).eraseAllUnderstanding();UniversalObservationStore.get(this).eraseAll();PersistentLifeMemoryStore.get(this).eraseAll();refresh();}).show();}
