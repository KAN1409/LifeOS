...
final class LifeOsIconView extends View {
    ...
    @Override protected void onDraw(Canvas c){super.onDraw(c);float size=Math.min(getWidth(),getHeight());if(size<=0)return;float ox=(getWidth()-size)/2f,oy=(getHeight()-size)/2f;c.save();c.translate(ox,oy);c.scale(size/24f,size/24f);p.setColor(color);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(2.05f);path.reset();drawIcon(c);c.restore();}
