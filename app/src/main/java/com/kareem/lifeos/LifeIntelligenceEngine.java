    ...
    static List<Result> search(Context context,String query,int limit){
        ...
        try(LifeDb db=new LifeDb(context)){
            for(AttentionStore.Item x:AttentionStore.get(context).openItems(100)){
                LifeDb.Event e=db.eventById(x.eventId);String who=e==null?"":LifeDb.personLabel(e);double score=120+x.priority+match(q,who+" "+x.summary+" "+x.reason)*30;all.add(new Result("Attention",who.isEmpty()?x.summary:who,x.summary+(x.reason.isEmpty()?"":" · "+x.reason),x.eventId,score));
            }
            for(LifeDb.Conversation c:db.recentConversations(100)){double m=match(q,c.label+" "+c.preview);if(q.isEmpty()||m>0)all.add(new Result("People",c.label,c.preview,c.latestEventId,65+m*28+recency(c.latestAt,now)));}
            ...
            for(LifeDb.Event e:db.recentEvents(300)){double m=match(q,e.title+" "+e.body+" "+e.threadKey+" "+LifeDb.friendlyApp(e.app));if(q.isEmpty()?all.size()<30:m>0)all.add(new Result(LifeDb.friendlyApp(e.app),e.title,e.body,e.id,45+m*32+recency(e.at,now)));}
        }
