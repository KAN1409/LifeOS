...
/** Conversation -> proactive summary -> message evidence -> original source. */
public final class ConversationDetailActivity extends Activity {
    ...
    private void render(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setBackgroundColor(BG);LinearLayout top=new LinearLayout(this);top.setGravity(Gravity.CENTER_VERTICAL);top.setPadding(dp(12),dp(10),dp(14),dp(8));top.setBackgroundColor(SURFACE);Button back=plain("‹");back.setContentDescription("Back");back.setOnClickListener(v->finish());top.addView(back,new LinearLayout.LayoutParams(dp(48),dp(48)));TextView title=text(seed==null?"Conversation":LifeDb.personLabel(seed),21,TEXT);title.setTypeface(Typeface.DEFAULT,Typeface.BOLD);title.setOnClickListener(v->openPerson());top.addView(title,new LinearLayout.LayoutParams(0,-2,1));Button person=plain("Person ›");person.setTextSize(12);person.setOnClickListener(v->openPerson());top.addView(person,new LinearLayout.LayoutParams(dp(88),dp(48)));root.addView(top);root.addView(divider());ScrollView sc=new ScrollView(this);LinearLayout body=new LinearLayout(this);body.setOrientation(LinearLayout.VERTICAL);body.setPadding(dp(16),dp(16),dp(16),dp(28));sc.addView(body);root.addView(sc,new LinearLayout.LayoutParams(-1,0,1));if(seed==null){body.addView(text("Conversation evidence is no longer available.",14,MUTED));setContentView(root);return;}
        ProactiveSummaryEngine.Result insight=ProactiveSummaryEngine.summarizeConversation(LifeDb.personLabel(seed),events);body.addView(text(events.size()+" captured items · "+app(seed.app),12,MUTED));LinearLayout summary=row();TextView sh=text("Summary",13,GREEN);sh.setTypeface(Typeface.DEFAULT,Typeface.BOLD);summary.addView(sh);String sig=ProactiveSummaryEngine.signalLine(insight);if(!sig.isEmpty()){TextView chips=text(sig.toUpperCase(),11,GREEN);chips.setTypeface(Typeface.DEFAULT,Typeface.BOLD);chips.setPadding(0,dp(7),0,0);summary.addView(chips);}TextView sv=text(insight.summary,15,TEXT);sv.setPadding(0,dp(8),0,0);summary.addView(sv);if(!insight.why.isEmpty()){TextView why=text(insight.why,13,GREEN);why.setPadding(0,dp(10),0,0);summary.addView(why);}body.addView(summary);
        LinearLayout actions=new LinearLayout(this);actions.setOrientation(LinearLayout.HORIZONTAL);Button open=primary("Open conversation");open.setOnClickListener(v->openSource(seed));actions.addView(open,new LinearLayout.LayoutParams(0,dp(48),1));Button personBtn=secondary("Person history");personBtn.setOnClickListener(v->openPerson());LinearLayout.LayoutParams pp=new LinearLayout.LayoutParams(0,dp(48),1);pp.setMargins(dp(8),0,0,0);actions.addView(personBtn,pp);body.addView(actions);
