...
final class SituationEngine {
    ...
    static List<Situation> build(LifeDb db,List<LifeDb.Loop> loops,List<PersistentActionQueue.Item> actions,long now){
        ...
        for(ProactiveFeedEngine.RankedConversation r:ranked){
            ...
            // High precision: a user-facing Situation must contain unresolved state.
            // Mere activity, a question mark, or a generic notification is not a Situation.
    ...
    static List<LifeDb.Event> evidence(LifeDb db,Situation s,int limit){
        ...
        // A standalone alert is one precise observation, even when Android supplied
        // an app-level thread key. Following that key would turn one bank/security
        // exception back into a noisy timeline of unrelated app notifications.
    ...
    private static String summary(LifeDb.Loop loop,int count){String kind=label(loop.kind);return kind+" · "+clip(loop.title,220)+(count>1?" · "+(count-1)+" more open item"+(count==2?"":"s"):"");}
    ...
    private static String label(String kind){if("financial_alert".equals(kind))return "Financial alert";if(blank(kind))return "Open item";return Character.toUpperCase(kind.charAt(0))+kind.substring(1);}
