...
final class SituationEngine {
    ...
    static List<Situation> build(LifeDb db,List<LifeDb.Loop> loops,List<PersistentActionQueue.Item> actions,long now){
        ...
        for(ProactiveFeedEngine.RankedConversation r:ranked){
            ...
            int score=r.score+attention*15+actionCount*8;String status=attention>0?"NEEDS ATTENTION":actionCount>0?"ACTION READY":r.summary.priority>0?"ACTIVE":"MONITORING";
            ...
            out.add(new Situation(key(c.app,c.threadKey,c.label),c.label,r.summary.summary,why,status,c.app,c.threadKey,c.latestEventId,c.latestAt,score,events.size(),attention,actionCount,new ArrayList<>(r.summary.signals),loopIds,evidenceIds,proposalIds));
        }
