...
final class SituationEngine {
    ...
    private static String summary(LifeDb.Loop loop,int count){String k=safe(loop.kind),prefix;if("request".equals(k)||"question".equals(k))prefix="Request";else if("commitment".equals(k))prefix="Open commitment";else if("appointment".equals(k)||"brain_calendar".equals(k))prefix="Scheduled plan";else if("deadline".equals(k))prefix="Dated obligation";else if("security".equals(k)||"brain_security".equals(k))prefix="Security change";else if("financial_alert".equals(k)||"brain_financial".equals(k))prefix="Financial exception";else if("brain_call".equals(k))prefix="Missed call";else if("brain_email".equals(k))prefix="Email";else if("brain_reminder".equals(k))prefix="Reminder";else if("brain_transaction".equals(k))prefix="Transaction";else if("brain_delivery".equals(k))prefix="Delivery";else prefix="Open item";return prefix+" · "+clip(loop.title,220)+(count>1?" · "+(count-1)+" related open item"+(count==2?"":"s"):"");}
    private static String why(String kind){String k=safe(kind);if("security".equals(k)||"brain_security".equals(k))return "An account-access change may need verification.";if("financial_alert".equals(k)||"brain_financial".equals(k))return "A financial exception may need prompt review.";if("brain_call".equals(k))return "A missed call may need a callback.";if("brain_email".equals(k))return "An email is waiting on you.";if("brain_reminder".equals(k))return "A reminder is waiting on you.";if("brain_transaction".equals(k))return "A transaction may need review.";if("brain_delivery".equals(k))return "A delivery update may need review.";if("deadline".equals(k))return "A dated obligation may be approaching.";if("appointment".equals(k)||"brain_calendar".equals(k))return "A scheduled plan may need preparation.";if("commitment".equals(k))return "A commitment is still open.";return "A direct request may still need a response.";}
    private static String displaySignal(String kind){String k=safe(kind);return k.startsWith("brain_")?k.substring(6).replace('_',' ').toUpperCase(Locale.ROOT):k.toUpperCase(Locale.ROOT);}
