...
public final class PersistentUnderstandingStore extends SQLiteOpenHelper {
    ...
    public synchronized void recordScreen(RawScreenSnapshot snapshot,List<MessageObservation> messages){
        ...
        try{ContentValues tree=new ContentValues();tree.put("dedupe_key","tree|"+snapshot.packageName+"|"+snapshot.capturedAt);tree.put("source","SCREEN_TREE");tree.put("thread",ConversationIdentityExtractor.fromSnapshot(snapshot));tree.put("type","SCREEN_SNAPSHOT");tree.put("direction","UNKNOWN");tree.put("text","");tree.put("observed_at",snapshot.capturedAt);tree.put("confidence",1.0);tree.put("payload",RawEvidenceSerializer.snapshot(snapshot));db.insertWithOnConflict("raw_evidence",null,tree,SQLiteDatabase.CONFLICT_IGNORE);if(messages!=null)for(MessageObservation m:messages)if(m!=null){ContentValues v=new ContentValues();v.put("dedupe_key","screen|"+m.observedAt+"|"+m.direction.name()+"|"+ReconciliationKey.normalize(m.text));v.put("source","SCREEN");v.put("thread",m.thread);v.put("type",m.type);v.put("direction",m.direction.name());v.put("text",m.text);v.put("observed_at",m.observedAt);v.put("confidence",m.confidence);v.put("payload","");db.insertWithOnConflict("raw_evidence",null,v,SQLiteDatabase.CONFLICT_IGNORE);}trimRaw(db);db.setTransactionSuccessful();}finally{db.endTransaction();}
    }
