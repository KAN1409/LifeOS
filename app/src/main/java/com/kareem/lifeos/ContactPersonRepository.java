...
final class ContactPersonRepository {
    ...
    static PersonObject load(Context c,String objectId){
        if(availability(c)!=Availability.OPERATIONAL)return null;String target=s(objectId),prefix="person:contact:";if(!target.startsWith(prefix))return null;String key=target.substring(prefix.length());if(key.isEmpty())return null;
        Cursor cur=null;try{
            Uri lookup=Uri.withAppendedPath(ContactsContract.Contacts.CONTENT_LOOKUP_URI,Uri.encode(key));
            cur=c.getContentResolver().query(lookup,new String[]{ContactsContract.Contacts._ID,ContactsContract.Contacts.LOOKUP_KEY,ContactsContract.Contacts.DISPLAY_NAME_PRIMARY},null,null,null);
            if(cur!=null&&cur.moveToFirst())return loadContact(c,cur.getLong(0),cur.getString(1),cur.getString(2));
            try{long numeric=Long.parseLong(key);if(cur!=null){cur.close();cur=null;}cur=c.getContentResolver().query(Uri.withAppendedPath(ContactsContract.Contacts.CONTENT_URI,String.valueOf(numeric)),new String[]{ContactsContract.Contacts._ID,ContactsContract.Contacts.LOOKUP_KEY,ContactsContract.Contacts.DISPLAY_NAME_PRIMARY},null,null,null);if(cur!=null&&cur.moveToFirst())return loadContact(c,cur.getLong(0),cur.getString(1),cur.getString(2));}catch(NumberFormatException ignored){}
        }catch(SecurityException ignored){}finally{if(cur!=null)cur.close();}return null;
    }
