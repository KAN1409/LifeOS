...
final class PhysicalStorageAudit {
    ...
    static JSONObject run(Context context) {
        ...
        try {
            ...
            for (String name : c.databaseList()) {
                ...
                j.put("path_private", f.getAbsolutePath().startsWith(privateRoot));
                j.put("size_bytes", f.exists() ? f.length() : 0);
            ...
            try {
                root.put("audit_error", String.valueOf(t));
        ...
        try {
            root.put("database_count", dbs.length());
            root.put("pass", pass);
            root.put("fail", fail);
    ...
    static void write(Context c, File out) throws Exception {
        JSONObject j = run(c);
