...
final class NotificationMeaningApplier {
    ...
    static boolean apply(Context context,LifeDb db,NotificationMeaning meaning,long eventId,long sourceAt){
        ...
        // Resolution is a relation to one exact older observation, never to an entire conversation.
        ...
        AttentionStore.Item current=attention.forEvent(eventId);if(current!=null&&(AttentionStore.HANDLED.equals(current.status)||AttentionStore.RESOLVED.equals(current.status)))return false;
        if(!compatible)return false;

        String kind=meaning.loopKind();String fingerprint="brain|"+meaning.sourceObservationId;long now=System.currentTimeMillis();
        sql.execSQL("INSERT OR IGNORE INTO open_loops(evidence_id,fingerprint,kind,title,due_at,confidence,priority,status,created_at) VALUES(?,?,?,?,0,?,?, 'open',?)",new Object[]{eventId,fingerprint,kind,meaning.summary,meaning.confidence,meaning.priority(),now});
        sql.execSQL("UPDATE open_loops SET kind=?,title=?,confidence=?,priority=?,status='open' WHERE fingerprint=?",new Object[]{kind,meaning.summary,meaning.confidence,meaning.priority(),fingerprint});
        return true;
    }
    ...
    private static boolean attentionCompatible(LifeDb.Event source,NotificationMeaning meaning){
        ...
        if("PERSON_CONVERSATION".equals(meaning.type))return EventSemantics.isPersonConversation(source)&&CanonicalSemanticPolicy.exactTargetEvidenceSupportsObligation(source,meaning);
    ...
    private static void reconcileExplicitResolution(LifeDb db,AttentionStore attention,NotificationMeaning meaning,long sourceAt){
        String priorId=meaning.resolvesObservationId==null?"":meaning.resolvesObservationId.trim();if(priorId.isEmpty())return;
        AttentionStore.Item prior=attention.forObservation(priorId);if(prior==null||!prior.isOpen())return;
